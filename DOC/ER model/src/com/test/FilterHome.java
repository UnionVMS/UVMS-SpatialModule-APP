...
public class FilterHome {
	...
	private static final Log log = LogFactory.getLog(FilterHome.class);
	...
	public void persist(Filter transientInstance) {
		log.debug("persisting Filter instance");
		try {
	...
	public Filter merge(Filter detachedInstance) {
		...
		try {
			Filter result = (Filter) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Filter findById(com.test.FilterId id) {
		log.debug("getting Filter instance with id: " + id);
		try {
			Filter instance = (Filter) sessionFactory.getCurrentSession().get(
					"com.test.Filter", id);
			if (instance == null) {
	...
	public List<Filter> findByExample(Filter instance) {
		log.debug("finding Filter instance by example");
		try {
			List<Filter> results = (List<Filter>) sessionFactory
					.getCurrentSession().createCriteria("com.test.Filter")
