...
public class LayerHome {
	...
	public Layer merge(Layer detachedInstance) {
		...
		try {
			Layer result = (Layer) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Layer findById(int id) {
		...
		try {
			Layer instance = (Layer) sessionFactory.getCurrentSession().get(
					"com.test.Layer", id);
			if (instance == null) {
	...
	public List<Layer> findByExample(Layer instance) {
		log.debug("finding Layer instance by example");
		try {
			List<Layer> results = (List<Layer>) sessionFactory
					.getCurrentSession().createCriteria("com.test.Layer")
