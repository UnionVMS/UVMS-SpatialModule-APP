...
public class AreaDao extends AbstractDAO {
    ...
    public List<AreaLayerDto> findSystemAreaLayerMapping() {
        Query query = getSession().getNamedQuery(QueryNameConstants.FIND_SYSTEM_AREA_LAYER); // TODO move to areaLocationTypeDAO
    ...
    public List<ServiceLayerDto> findServiceLayerBySubType(List<String> subAreaTypes, boolean isWithBing) {
        if (isWithBing) {
            return createNamedQueryWithParameterList(QueryNameConstants.FIND_SERVICE_LAYER_BY_SUBTYPE, SUB_TYPE, subAreaTypes, ServiceLayerDto.class).list(); // TODO moveto serviceLayerDao
        } else {
    ...
    private Query createNamedNativeQuery(String nativeQueryString, String wktPoint, int crs) {
        ...
        query.setParameter("shape", "SRID=" + crs + ";" + wktPoint); // TODO DaoTest Oracle
        return query;
    }

    private <T> Query createNamedQueryWithParameterList(String nativeQuery, String parameterName, List<? extends Object> parameters, Class<T> dtoClass) {
