...
@NamedQueries({
        ...
        @NamedQuery(name = PortEntity2.CLOSEST_PORT, query = "select new eu.europa.ec.fisheries.uvms.spatial.service.Service2.dto.PortDistanceInfoDto(p, distance(p.geom, :point, true)) from PortEntity2 p where enabled=true AND dWithin(p.geom, :point, 22224.0, true) = true order by distance(geom ,:point, true) "),
