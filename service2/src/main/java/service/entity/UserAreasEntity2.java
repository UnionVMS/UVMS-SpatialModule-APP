...
@NamedQueries({
        ...
        @NamedQuery(name = UserAreasEntity2.DISABLE, query = "UPDATE UserAreasEntity SET enabled = false"),
        @NamedQuery(name = UserAreasEntity2.BY_INTERSECT, query = "FROM UserAreasEntity WHERE intersects(geom, :shape) = true AND enabled = true"),
        @NamedQuery(name = UserAreasEntity2.SEARCH_USERAREA, query = "FROM UserAreasEntity where (upper(name) like :name OR upper(code) like :code) AND enabled=true GROUP BY gid"),
        @NamedQuery(name = UserAreasEntity2.SEARCH_USERAREA_NAMES_BY_CODE, query = "From UserAreasEntity where code in (SELECT distinct(code) from UserAreasEntity where (upper(name) like :name OR upper(code) like :code) AND enabled=true GROUP BY gid)"),
